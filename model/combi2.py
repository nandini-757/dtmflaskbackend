...
df_crop_recommendation = pd.read_csv('model/cropre.csv')
...
crop_features = ['N', 'P', 'K','temperature', 'rainfall', 'ph', 'humidity']
    ...
    for feature, weight in contributions.items():
        if weight > 0:
            ...
            if feature == "rainfall":
                ...
                explanation += f"The nitrogen (N) level is suitable for promoting the growth of {crop_name}.\n"
            elif feature == "P":
                explanation += f"The phosphorus (P) level is beneficial, supporting the development of {crop_name}.\n"
            elif feature == "K":
                explanation += f"The potassium (K) level is adequate, enhancing the resilience of {crop_name}.\n"
            else:
            ...
            if feature == "rainfall":
                ...
                explanation += f"Low nitrogen (N) levels might restrict the growth of {crop_name}.\n"
            elif feature == "P":
                explanation += f"Inadequate phosphorus (P) levels may hinder the development of {crop_name}.\n"
            elif feature == "K":
                explanation += f"Potassium (K) levels are insufficient, possibly affecting {crop_name}’s resilience.\n"
            else:
...
def predict_crop():
    ...
    crop_input = pd.DataFrame([[N,P,K,temperature, rainfall, ph, humidity]], columns=crop_features)
